...
QList<QPair<QString,QVariant> > MAVLinkDecoder::receiveMessage( mavlink_message_t message)
{
    ...
    if (message.msgid == MAVLINK_MSG_ID_SYSTEM_TIME)
    {
        ...
        onboardToGCSUnixTimeOffsetAndDelay[message.sysid] = static_cast<qint64>(My_GS::groundTimeMilliseconds() - (timebase.time_unix_usec+500)/1000);
    }
    else
    {
...
quint64 MAVLinkDecoder::getUnixTimeFromMs(int systemID, quint64 time)
{
    ...
    if (time == 0)
    {
        ret = My_GS::groundTimeMilliseconds() - onboardToGCSUnixTimeOffsetAndDelay.value(systemID,0);
    }
...
#endif
    {
        if (!onboardTimeOffset.contains(systemID) || (time < firstOnboardTime.value(systemID,0)-100))
        {
            ...
            onboardTimeOffset.insert(systemID,My_GS::groundTimeMilliseconds() - time);
        }
